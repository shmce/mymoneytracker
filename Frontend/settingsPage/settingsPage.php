...
<head>
    ...
    <title>MyMoney Tracker Settings</title>
    <!-- 1. Load Tailwind CSS -->
...
<body class="bg-gray-100">
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            <div class="bg-white p-6 rounded-lg shadow-sm">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button @click="toggleEdit" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition">{{ isEditing ? 'Save' : 'Edit' }}</button>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">First Name</label>
                        <input type="text" v-model="form.firstName" :disabled="!isEditing" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Your First Name" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Last Name</label>
                        <input type="text" v-model="form.lastName" :disabled="!isEditing" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Your Last Name" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Gender</label>
                        <select v-model="form.gender" :disabled="!isEditing" :class="['w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500', { 'show-dropdown': isEditing }]">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <select v-model="form.dob.month" :disabled="!isEditing" :class="['px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500', { 'show-dropdown': isEditing }]">
                                <option value="">Month</option>
                                <option v-for="m in months" :key="m.value" :value="m.value">{{ m.label }}</option>
                            </select>
                            <select v-model="form.dob.day" :disabled="!isEditing" :class="['px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500', { 'show-dropdown': isEditing }]">
                                <option value="">Day</option>
                                <option v-for="d in days" :key="d" :value="d">{{ d }}</option>
                            </select>
                            <select v-model="form.dob.year" :disabled="!isEditing" :class="['px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500', { 'show-dropdown': isEditing }]">
                ...
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">My Email Address</label>
                    <div class="flex items-center bg-gray-50 p-3 rounded-lg mb-2">
                        <i data-lucide="mail" class="w-5 h-5 text-green-600 mr-3"></i>
                        <div class="flex-1">
                            <input v-if="isEditing" v-model="form.email" type="email" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="you@example.com" />
                            <span v-else class="text-gray-800">{{ form.email || 'No email set' }}</span>
                        </div>
                    </div>
                </div>

                <div class="flex justify-center">
                    <button id="logoutBtn" class="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700 transition">Logout</button>
                </div>
                <!-- Logout Confirmation Modal -->
                <div id="logoutModal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black bg-opacity-40">
                    <div class="bg-white rounded-lg w-full max-w-sm p-6 shadow-lg">
                        <h3 class="text-lg font-semibold mb-2">Confirm Logout</h3>
                        <p class="text-sm text-gray-700 mb-4">Are you sure you want to log out?</p>
                        <div class="flex justify-end gap-2 mt-4">
                            <button id="cancelLogout" class="px-4 py-2 rounded bg-gray-100">Cancel</button>
                            <button id="confirmLogout" class="px-4 py-2 rounded bg-red-600 text-white">Logout</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <!-- Inline script to control logout modal and redirect -->
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            confirm.addEventListener('click', () => {
                window.location.href = '../loginPage/Logout.php';
            });
        });
        </script>
